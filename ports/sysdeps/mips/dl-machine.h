...
/*
static __attribute__ ((nomips16)) inline int
get_frmode (void)
{
  int frmode;
			...
			struct link_map *map)
{
  ...
  for (nsid = 0; nsid < DL_NNS; ++nsid)
	...
	switch (l->l_mach.fpabi)
	  ...
	  case Val_GNU_MIPS_ABI_FP_DOUBLE:
		...
		|| (req_abi == Val_GNU_MIPS_ABI_FP_64C
		    && (GLRO(dl_hwcap) & HWCAP_MIPS_UFRC)))
	      success = true;
	    fr0_required = true;
	    break;
	  case Val_GNU_MIPS_ABI_FP_XX:
  ...
  if ((GLRO(dl_hwcap) & HWCAP_MIPS_FR1) == 0)
    {
      if (req_abi == Val_GNU_MIPS_ABI_FP_64C
	  || req_abi == Val_GNU_MIPS_ABI_FP_64)
	{
	...
	fr0_required = true;
      ...
      if ((GLRO(dl_hwcap) & HWCAP_MIPS_UFRC) && fr1_required)
	{
	  if (fr0_required)
	    switch_frcmode_to (1);
	  else
  ...
  else
    {
      if (req_abi == Val_GNU_MIPS_ABI_FP_DOUBLE)
	{
	  if ((GLRO(dl_hwcap) & HWCAP_MIPS_UFRC) == 0)
	    {
	      if (__glibc_unlikely (GLRO(dl_debug_mask) & DL_DEBUG_LIBS))
		_dl_debug_printf (
		    "   requires FR0 compatibility mode but UFRC is not "
		    "supported\n");
	      return false;
	    }
	  switch_frcmode_to (1);
